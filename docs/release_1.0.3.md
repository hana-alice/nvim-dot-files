# hana-alice/nvim 1.0.3 — Patch Release

> Version: 1.0.3
> Repo:    https://github.com/hana-alice/nvim-dot-files
> Platform: Windows 11 + Neovide GUI (primary) / WSL2 (secondary)
> Date:    2026-05-25
> Type:    Patch release (DAP overhaul + docs)

---

## One-line summary

End-to-end migration from **codelldb -> lldb-dap 22.1.6** for the UE
Android attach route, plus a F10 client-side step throttle that fixes
"hold F10 -> app crashes + debugger disconnects" on platform-mode
Android attach. dap-ui layout rebuilt around a VS-style left rail +
right-bottom tab host. New `docs/architecture-vs-lazyvim.md` documents
every divergence from the upstream LazyVim baseline.

---

## What changed (from 1.0.2)

### DAP stack — codelldb -> lldb-dap 22.1.6

- **Adapter migration.** All routes (Android / Win64 / Linux / macOS /
  iOS) now use the LLVM-shipped `lldb-dap` executable instead of the
  vscode-lldb `codelldb` adapter. Cuts our private side-load
  (`C:\tools\lldb-21\bin`) entirely; lldb-dap ships with the same
...
- **F10 client throttle.** Mirror of `request_dap_continue`'s
  ...
  spammed step keys no longer back-to-back the adapter; on
  platform-mode Android attach this used to crash the inferior +
  disconnect DAP. 750 ms watchdog releases the keymap if a stopped
  event never arrives. High-frequency rejects stay silent; low-
  frequency rejects (session running / state machine confused)
  surface a 2 s-throttled vim.notify pointing at `<F6>` or
  `:UEDAPDiag`.
- **Multi-thread stopped focus fix.** `before.event_stopped` listener
  seeds `expected_focus_thread_id` so non-`threadCausedFocus` events
  don't overwrite `stopped_thread_id` between F10 presses. Without
  this, the next step ran on the wrong thread.
- **Per-thread stopped burst suppression.** lldb-dap 22 reports
  stopOnEntry as ~165 stopped events on a UE Android target (one per
  thread). `dap.defaults.lldb.auto_continue_if_many_stopped=false`
  keeps nvim-dap from issuing per-thread continues that wedge the
  session.
- **Blob-safe is_file.** `vim.fn.filereadable` raises E976 on Vim
  Blobs. lldb-dap 22 occasionally encodes `source.path` as msgpack
  `bin8` -> Blob. `lua/ue/core/fs.lua::is_file` now tostring-coerces
  and pcall-backstops; resolves an E976 flood in the stackTrace
  listener.
- **stale popup cleanup.** snacks.notifier accumulates active toasts
  forever; `M.attach()` now hides all per-id toasts so the new
  session's diagnostics aren't drowned in pre-fix residue.

### dap-ui layout

- Left debug rail only: scopes / stacks / watches always docked left.
- Drop dap-ui's `bottom` layout (`botright split` spans the full
  width including under the rail). Replaced with a custom bottom
  split parented to the saved code window — bottom aligns with code
  instead of crossing the rail.
- Right-bottom tab host: `:UEDAPTab repl|console|breakpoints|logcat`,
  cycle with `:UEDAPNextTab` / `:UEDAPPrevTab`, direct keys
  `<leader>d1..d4` and `<leader>d]` / `<leader>d[`.
- Logcat moves inside the tab host instead of its own vertical split.

### Documentation

- **New `docs/architecture-vs-lazyvim.md`** — 9-section audit of every
  divergence from upstream LazyVim. Each section follows the same
  shape: LazyVim default -> what we do instead -> why -> where.
  Covers UE C++ workflow / ue_goto / workaround registry / snacks
  tuning / Windows ergonomics / DAP stack / code_search trigram /
  rotating logger / self-verifiable modules.

### Engineering notes

- **lldb-dap 22.1.6 platform-mode 翻案.** Earlier in this cycle we
  bounced the migration once on the assumption that
  `--notify true` storms in platform-android mode crashed the
  inferior (see TOOLING.md `STATUS_STACK_BUFFER_OVERRUN` history).
  Probe v12 with `--notify false` PASSED end-to-end; codelldb is
  still hangable, just throttled. See the detailed log below.
- **F10 attribution honesty.** The "back-to-back step crashes
  lldb-dap inferior" theory is plausible but unconfirmed upstream
  (6 LLVM Issues queries returned zero direct hits). The
  state-machine asymmetry fix (request_step missing the guard
  request_dap_continue has) is the real, code-readable bug; the
  attribution to lldb-dap 22 stays in the "suspected" column until
  a logcat / LLDBDAP_LOG capture confirms it.
- **uv.spawn env array form.** nvim-dap session.lua L1601 passes our
  env table directly to uv.spawn without normalization. uv.spawn
  expects an ARRAY of `"K=V"` strings, not a hash map. Our
  `_lldb_dap_env()` now builds the array form so future adapter
  env additions stay correct.

---

## Detailed log

### 2026-05-25 — docs: split architecture-vs-lazyvim out of README

**Task** — 用户要求"写明白本配置之于 LazyVim 上游的特性，分模块写
清楚优势、实现方式、架构"。

**Implemented**
- 新文档 `docs/architecture-vs-lazyvim.md`（21 KB / 9 个模块章节）
  - 每模块统一结构：LazyVim 默认 → 我们改成怎样 → 为什么 → 在哪里
  - 覆盖：UE C++ workflow / ue_goto / workaround registry /
    snacks tuning / Windows-first ergonomics / DAP stack /
    code_search trigram / rotating logger / self-verifiable modules
  - 末尾 "What we deliberately don't do" 段记录五条主动否决项
    （no telescope / no which-key cheatsheet / no in-config copilot /
    no global vim.lsp.handlers / no mason auto-install）
- README 加 architecture-vs-lazyvim.md 的入口链接
- 修正前两条 entry 误用 `## ` 顶级标题，统一改为 `###`

**Pitfalls / Gotchas**
- hermes write_file 工具在本机 terminal session 卡死状态下也连带
  失败（共享同一个坏 cd 的 shell 前缀），所有 file 写入改走 Python
  open(..., 'w', newline='\n') 绕过；.gitattributes 已强 eol=lf
  所以工作树写 LF 不会被 git 翻动。

**Validation**
- 内部代号字面量扫描干净。

**Follow-ups**
- 等 architecture-symbol-resolution.md 里那些"v1.0.0 之前 commit"的
  cross-link 老化后做一次大梳理。

### 2026-05-25 — release: bump baseline to v1.0.3

Slice the v1.0.3 entry into docs/release_1.0.3.md and reset the
rolling Unreleased section to empty.

Key changes since v1.0.2:
  - DAP stack overhaul: codelldb -> lldb-dap 22.1.6 platform-mode
    Android attach, including a F10 client-side step throttle that
    stops back-to-back step requests from disconnecting the inferior.
  - dap-ui layout rebuilt: left rail + right-bottom tab host (REPL /
    console / breakpoints / logcat).
  - UE Android attach is now one-command via packageInfo.txt
    auto-discovery.
  - New `docs/architecture-vs-lazyvim.md` documenting every divergence
    from the upstream LazyVim baseline.
...
- Headless RPC probe (`/c/Users/lizeqiang/AppData/Local/Temp/probe_picker2_run.lua`)
  ...
  - Case A: ctx=nil, cwd=E:/sample/zeqiang_sample_3.4 →
    package=`com.example.mygame`, sym=`Client_Symbols_v<code>/.../libUE4.so`, 0 prompts
  - Case B: ctx={project_root="E:/sample/zeqiang_sample_3.4"} → 同上, 0 prompts
  - Case C: ctx={engine_root="E:/sample/zeqiang_sample_3.3"} 但 cwd=3.4 → fallback 到
