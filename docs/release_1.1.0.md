# hana-alice/nvim 1.1.0 — Minor Release

> Version: 1.1.0
> Repo:    https://github.com/hana-alice/nvim-dot-files
> Platform: Windows 11 + Neovide GUI (primary) / WSL2 (secondary)
> Date:    2026-07-24
> Type:    Minor release (Android wait-for-debugger launch, csearch smart incremental, cross-drive clangd fix, perf/diag hardening)

---

## One-line summary

Android DAP gains **wait-for-debugger launch** (Android Studio debug-button
semantics — catches earliest-init crashes) plus a fix for the liveness-poller
main-loop stall train; UEPrepare's csearch build turns **smart-incremental**
(diff-driven skip/add/reset instead of full rebuild); the cross-drive
project-root `.clangd` gap that made clangd background-index half the CDB
after every UEPrepare is closed. Also: search-system refactor (csearch
de-platformed), single-writer csearch.idx, content-fingerprint freshness,
task registry, notification history, stall probe, Porcelain theme, AI
knowledge-base restructure (AGENTS.md single-source).

---

## Working log (sliced from docs/changelog.md Unreleased, newest first)

### 2026-07-24 — fix(clangd): UEPrepare 后 clangd 吃满 CPU/内存 —— 跨盘 project root 缺 .clangd 致 background-index 全量跑

**Task** — 用户报「UEPrepare 之后 clangd index 占用非常多 CPU 和内存」。

**Root cause** — clangd 的 `.clangd` 配置按**源文件路径向上查找**。本仓 `sync_dot_clangd`
只在 **engine root**（D: 盘引擎树）落 `.clangd`（含 `Index.External` +
`Background: Skip`）；但当前工程 project 在**另一块盘**（E: 盘 project root），CDB 16178 条
里 **8782 条（54%）是 E: 侧 TU**。E: 的源文件向上永远找不到 D: 的 `.clangd` →
这半个 CDB 没有 `Background: Skip` → 每次 UEPrepare 重生成 CDB 后，clangd
`--background-index -j=24` 对 ~8.8k 个 UE TU 全量后台索引（每 TU preamble 1.5–3GB
峰值）。实证：`%LocalAppData%/clangd/index`（compile-commands 目录外文件的回落
shard 目录）堆了 **14,556 个 shard、105MB**，mtime 一直到 15:04 还在写。

**Implemented**
- `lua/ue.lua`：`sync_dot_clangd` 拆为 `sync_one_dot_clangd(path, file, mount)`（单文件
  surgical 编辑，原逻辑不变）+ 编排层：engine root 照旧；**project root 在引擎树外时
  同步落第二份 `.clangd`**（同一 active idx 文件、mount=project root、`Background: Skip`）；
  树内 project 跳过（向上查找已命中 engine 的）。幂等（unchanged 不写盘、防 clangd
  重载放大）。
- 立即止血（不等下次 index build）：手写 project root 的 `.clangd` 指向
  active idx；清掉 `%LocalAppData%/clangd/index` 的 14.5k 陈旧 shard（纯缓存）。
- 测试：`tests/cases/ue_api_spec.lua` 新增 4 例——新建内容形状 / 幂等 unchanged /
  跨盘双写（同 idx、各自 mount）/ 树内不重复落。

**Pitfalls / Gotchas**
- `.clangd` 的 `If.PathMatch` 用户级 config（`%LocalAppData%/clangd/config.yaml`）只配了
  旧 UnrealEngine 路径——每换一个 engine/project root 手工加一条不可持续；per-root
  `.clangd` 由 pipeline 自动维护才是正解。
- Lua 双引号串里 `\i`/`\m` 是非法转义（loadfile 直接炸「invalid escape sequence」）；
  spec 里的 Windows 路径字面量用 `[[D:\idx\a.idx]]` 长字符串写。
- shard 回落目录的位置很有诊断价值：TU 在 compile-commands 目录**内** → shard 进
  `<root>/.cache/clangd/index`；**外** → 进 `%LocalAppData%/clangd/index`。后者暴涨
  = 有一大片 TU 没被任何 `.clangd` 覆盖。

**Validation**
- `ue_api` filter → exit 0（新增 4 例全绿）；全量 `nvim --headless -l tests/run.lua` → exit 0。
- 止血侧：E: root `.clangd` 已落、stale shard 已清；重启 nvim/clangd 后观察任务管理器
  应无 clangd 高 CPU 常驻（待用户确认）。

**Follow-ups**
- 重启 nvim 后确认 clangd 稳态内存（预期 ~2–4GB 而非 17GB+）且 `gd` 到 E: 侧符号正常。
- 换 project（:UESetProject）后确认新 project root 自动落 `.clangd`（下次 index build
  成功即触发 sync_dot_clangd）。

### 2026-07-24 — perf(csearch): UEPrepare 智能增量构建（D11）——stale 不再等于全量重建

**Task** — 用户反馈「每次 UEPrepare csearch 都全量重建很慢，希望至少增量」。D10 之后
freshness 判定准确了，但每个 stale 判定（哪怕只加了 3 个文件）都触发全量 `-reset`
重建（UE 树数分钟）。cindex 原生支持增量 `add`；缺的是「上次索引喂了哪些文件」的
diff 基准。

**Implemented**
- `lua/ue.lua` 新增 D11 智能增量层：
  - `CORE_RT.csearch_snapshot_path(ctx)` → `<csearch_idx>.files`：每次构建成功后把喂给
    cindex 的绝对路径清单快照到 idx 旁（与 idx 同目录 → C5b 按平台分路径语义免费继承）。
  - `CORE_RT.csearch_build_mode(stats)` 纯决策：forced/无快照 → reset；**有删除 → reset**
    （cindex 无删除能力，ghost 命中是正确性问题）；added+dirty==0 → **skip**；
    delta >30%（`CSEARCH_ADD_RATIO_MAX`）→ reset（切分支场景）；否则 → **add 只喂 delta**。
    dirty = watcher persistent set（改动的既有文件 re-add 刷新 trigram，也走便宜路径）。
  - `CORE_RT.csearch_smart_build(ctx, cs_ctx, abs_list, cb)` 单一入口：diff + 决策 +
    add 失败自动回退一次 reset（D9 guard 拒绝 unusable idx 时不再死路）+ 成功刷新快照。
  - 三条 prepare 路径（sync L8306 / cache fast-path L8546 / cold-full L8863）的裸
    `build_index` 调用全部替换为 `csearch_smart_build`；begin/done 串行闸与
    `on_full_csearch_success`（清 dirty + 写指纹）职责不变。
  - 通知区分模式：`✓ csearch incremental: +N added, M dirty — ...` / `already current` /
    `rebuilt`。
- 与 D9 关系：写者仍只有 prepare 家族（smart_build 是 prepare 的实现细节，不是新写者）；
  watcher 仍是记账员；`UEPrepareIncremental` 手动挡保留不变。
- `docs/architecture/grep-cache-invalidation.md` 新增 **D11** 小节（机制/决策表/边界）。
- `tests/cases/csearch_build_guard_spec.lua` 新增 2 组 describe：
  - `csearch_build_mode` 决策矩阵（forced/无快照/删除/零变化/小增量/超阈值 6 例）。
  - smart_build 端到端（mock `cs.build_index`）：首建 reset+落快照 / 集合不变 skip 且
    零 cindex 调用 / 小增量 add 只喂 delta 且快照同步 / 删除走 reset / add 失败自动回退
    reset（先 add 后 reset 两次调用序列）。

**Pitfalls / Gotchas**
- **删除必须全量**：cindex merge 只能加不能减；留 ghost 会让 `<leader>/` 命中已删文件
  ——比慢更糟，正确性优先。
- 30% 阈值下 add 的 merge 成本仍显著低于全量；超过后（典型 git 切分支）reset 更快也更干净。
- 测试里 add-fallback 用例的基数要够大（1/11≈9% < 30%），否则 delta 比例直接把决策推到
  reset，测不到回退路径（首版踩过：expected 2 calls got 1）。
- 快照在 skip 路径也要刷新——新清单可能只是排序/大小写归一变化，内容集合等价但 bytes
  不同；不刷会导致每次都重新 diff 出伪 delta。

**Validation**
- `csearch_build_guard` filter → exit 0（新增 11 例全绿）。
- `freshness` / `grep_cache` / `ue_api` filter → exit 0。
- 全量 `nvim --headless -l tests/run.lua` → exit 0 全绿。
- 真实工程收益待下次 :UEPrepare 观察（预期：日常小改动秒级 add / 集合不变直接 skip）。

**Follow-ups**
- 真实 UE 工程实测一轮：改 2 个文件 → :UEPrepare → 应报 `✓ csearch incremental: +N`；
  git 切大分支 → 应报 reset（delta 超阈值或有删除）。

### 2026-07-24 — perf(dap.android): liveness poller 同步 adb 探测致全天卡顿 → 改异步（K40）

**Task** — 用户报「使用过程有卡顿、stall log 记录了很多」。stall_probe 证据：2026-07-24
10:47–16:49 连续 ~50 stalls/min（共 19,366 条 WARN），p50≈410ms、p90≈998ms，buffer 无关
（OpenGLES.h / cmd.exe / picker 全中招）——典型「后台定时器阻塞」特征而非按键动作阻塞。

**Root cause** — `lua/ue/dap/android.lua` `_start_liveness_poller`：每 1.5s 在
`vim.schedule_wrap`（主循环）里同步调 `pidof()` → `vim.fn.system({adb, ..., "pidof"})`。
adb USB 往返 200–1000ms（调试会话期间 lldb-dap 抢占 adb server 更慢），每次轮询都把
主循环钉住一个往返；从 10:47 attach 起卡到 16:49，与 stall 窗口完全吻合。违反 P6。

**Implemented**
- `lua/ue/dap/android.lua` `_start_liveness_poller` 重写：
  - timer 回调降级为 fast-event 纯 spawn（`vim.system` async callback），状态处理
    `vim.schedule` 回主循环 —— 主循环零阻塞。
  - `in_flight` 防探测堆积（adb 比轮询周期慢时不叠加）。
  - 回调先复核 `M._liveness_timer == timer` 与 session/pid 未变（探测在飞期间可能已
    stop/替换），双保险防 stale 回调打到新会话。
  - miss 容忍 / 2 次判死 / stop+notify 语义不变。
- `docs/CONSTRAINTS.md` 新增 **K40**：uv timer 回调禁止同步 `vim.fn.system`；周期探测
  一律 async + in_flight + 回调复核。诊断入口 `:StallReport` 按分钟聚类。

**Pitfalls / Gotchas**
- stall 记录的 buf/keys 是「主循环解冻瞬间」的前台状态，**不是**元凶——分析时先看
  「与 buffer/按键是否无关 + 是否等间隔 train」，无关且等间隔 = 后台 timer 阻塞。
- `vim.system` 的 on_exit 跑在 libuv 线程，碰 nvim 状态前必须 `vim.schedule`。

**Validation**
- `nvim --headless -l tests/run.lua dap` → exit 0；`stall_probe` filter → exit 0。
- 运行时验证需下次真机调试会话观察 `:StallReport`（预期 stall train 消失）。

**Follow-ups**
- 下次 Android 调试会话跑 `:StallProbe status` + `:StallReport` 确认修复生效。

### 2026-07-24 — feat(dap.android): wait-for-debugger launch（AS debug 按钮语义）+ chmod EPERM 残留修复

**Task** — 用户要 Android「attach on launch」：太早的 crash（JNI_OnLoad / 引擎 PreInit）现有
「monkey 启完再 attach」路径永远抓不到；另修 log 里的
`chmod '/data/local/tmp/lldb-server' to 0755: Operation not permitted`（2026-07-24 两次 ERROR）。

**Implemented**
- `lua/ue/dap/android.lua`
  - `M.launch` 重写为 wait-for-debugger 语义：`am force-stop` → `am set-debug-app -w <pkg>` →
    monkey start（进程冻结在 JDWP "Waiting for debugger" 闸门，libUE4.so 尚未加载）→ 拿 pid 后
    立即 `am clear-debug-app`（one-shot，不粘住后续手动启动）→ 走既有 K30 platform attach。
    `set-debug-app` 失败按用户政策**直接报错停止**（不静默回退），原因记入 diag log。
  - `wait_launch_device_steps` / `jdb_connect_argv`：纯命令构造 helper（headless 可测）。
  - `arm_wait_mode_followup`：一次性 `listeners.after.event_continued` 钩子，用户第一次 F5 时
    (a) `start_jdwp_release` — `adb forward tcp:<port> jdwp:<pid>` + `jdb -connect SocketAttach`
    释放 JDWP 闸门（jdb 常驻无害，cleanup 时 jobstop）；(b) `M._start_late_rebase_poller`。
  - `M._start_late_rebase_poller`：wait 模式下 attach 时 libUE4.so 未映射、显式 slide 发不出
    （K37 语境属预期而非失败）；poller 用 `vim.system` 异步轮询 `/proc/<pid>/maps`（P6 不阻塞），
    模块出现后经 lldb-dap evaluate backtick 通道（K36 正解通道）补发
    `target modules load --slide 0x<base>`，并回读 `breakpoint list` 入 diag log。
    slide 仍是字符串拼接（P7/K4）。~60s 超时。
  - `wait_notice` + `M._wait_notice_seen`：wait 路径所有失败**每会话只记一次**
    （notify + ue-dap-bp-diag.log 带完整上下文），每次 launch 重置 —— 用户政策「报错并记录
    原因，不重复记录，方便复查」。
  - `_finalize_session` ASLR 分支：wait 模式下 base 未解析只写 diag log，不再弹 WARN。
  - `_cleanup_device_side`：新增停 late-rebase poller、disarm wait 钩子、jobstop jdb、
    `am clear-debug-app`（防粘）、移除 jdwp forward。
  - `ensure_lldb_server_pushed` 重写：`lldb_server_stage_plan(size_matches, is_executable)`
    纯决策（reuse / chmod / repush）。root-owned 残留（旧 adb root 会话推的文件）同尺寸且已
    0755 → 直接 reuse 不 chmod；尺寸不符 → 先 `rm -f`（目录 shell-owned 可 unlink root 文件）
    再 push；chmod EPERM 但 `test -x` 通过 → WARN 继续而非 ERROR 中止。
  - `read_so_base_hex` 拆出纯解析 `parse_maps_base_hex`（headless 可测）；`pick_port` 拆出
    `alloc_free_port`（jdwp forward 复用）。
- `lua/ue/config.lua`：新增 `dap.jdb_path`（nil → PATH → JAVA_HOME/bin 探测）。
- `tests/cases/dap_spec.lua`：新增 3 组 describe —— stage_plan 三态、parse_maps_base_hex
  （64 位 hex 保形 / '.' 不作通配 / nil-safe）、wait-launch 命令形状（`-w` 必须在、
  clear-debug-app 必须在、jdb SocketAttach 串）。

**Pitfalls / Gotchas**
- `/data/local/tmp/lldb-server` 为 root:root 时 shell 用户 chmod 必 EPERM，但 unlink 看的是
  **父目录**权限（shell-owned）→ rm-then-push 是正解；同尺寸已可执行则连 chmod 都不需要。
- wait 模式 attach 时读 maps 拿不到 libUE4.so 是**预期**（模块未加载）——不能把它当 K37 失败；
  K37 的显式 slide 改为「晚到」而非「缺席」，经 evaluate 通道（K36 已实证该通道可下命令）。
- JDWP 闸门必须在 lldb attach **之后**、用户 F5 时释放：太早释放 = 回到旧语义漏掉早期 init；
  挂在 `event_continued`（首次）正好是「断点已布好、用户主动放行」的时点。
- `am set-debug-app -w` 是全局粘性标志：任何退出路径都要 `clear-debug-app`（launch 拿到 pid
  即清 + cleanup 兜底再清）。

**Validation**
- `nvim --headless -l tests/run.lua dap` → exit 0（含新增 3 组用例）。
- `platform` / `ue_config` filter → exit 0。
- 全量 `nvim --headless -l tests/run.lua` → exit 0 全绿。
- 真机 wait-launch 端到端（冻结 → attach → F5 → 早期断点命中）待用户实测（记 Follow-ups）。

**Follow-ups**
- 真机验证：`:UEDAPLaunch android` → 观察 "Waiting for debugger" → F5 → 早期断点命中 +
  late-rebase poller 在 diag log 里补发 slide。
- 若 jdb 释放时机太晚（部分 ROM 在 attach 前就要求 JDWP 应答），考虑把释放提前到
  `event_initialized` 并复测早期断点覆盖。
...
- Direct `rg -n -i -F "r.useLandscape"` over `D:/project/uetemp/Engine` + `E:/sample/zeqiang_sample_3.5/Source/Client` → 14 hits.
...
**Task** — User reported `<space>da` / `:UEDAPAttach android` error and asked to read the local docs plus config rules before fixing. The captured DAP protocol log showed `process attach --pid ...` failed with `lost connection`, followed by `target symbols add ... does not match any existing module`; the earlier probe also showed context project_root was `E:/sample/zeqiang_sample_3.4/Source/Client`.
...
- Headless explicit-ctx probe:
  - `effective_project_root=E:/sample/zeqiang_sample_3.4/Source/Client`
  - `package=<android-package>`
  - `symbol=E:/sample/zeqiang_sample_3.4/Source/Client/Binaries/Android/Client_Symbols_v170300916/Client-arm64/libUE4.so`
  - attach config includes `target symbols add` for that exact host DWARF file and sourceMap `D:\project\uetemp -> E:/sample/zeqiang_sample_3.4/Source/Client`.
...
**Task** — The `<space><space>` picker and `:UEPrepare` were silently returning 5/14-era results on `D:\project\uetemp` after `z uetemp` + `:UESetProject E:\sample\zeqiang_sample_3.4` + build. Three compounding bugs hid behind one symptom.
...
**Follow-ups**
- Run `:UESetProject E:\sample\zeqiang_sample_3.4` on `D:\project\uetemp` and confirm: WARN toast with invalidation summary, then the picker shows freshness banner, then `:UEPrepare` / `:UEPrepareIncremental` both work.
...
**Issue**
用户希望 csearch 收 .ini / .ts / .cs，结果发现 csearch grep 完全搜不到 `E:\sample\zeqiang_sample_3.4\Source\Client\CSharpScript\...` 下的内容（GameSettingsConfig 0 hits），也搜不到 .ini 里的常见 token（AndroidPackageName 在 .ini 0 hits / .cpp 40 hits）。但 workspace_all.files 里这些路径都在。

**Root cause**
跨盘符 (project 在 E:，engine 在 D:) 导致 `workspace_root()` 退化为 `engine_root = D:/project/uetemp`，然后 `_ufs.relative_to(root, project_abs_path)` 因 `path_has_prefix` false → **返回原绝对路径**，所以 workspace_all 列表里 project 侧文件全是绝对路径 `E:/sample/...` 开头。但 ue.lua 三处构造 cindex `csearch_filelist.txt` 时无脑 `fout:write(cs_root, "/", rel, "\n")`，对 project 文件拼出 `D:/project/uetemp/E:/sample/.../GameSettingsConfig.cs` 这种荒诞路径 → cindex `os.Stat` fail → skipped++ → **整个 project 树（含 CSharpScript / TypeScript / Source / Config / Plugins）在 csearch 索引里完全不存在**。
...
- `is_absolute_path("E:/sample/x.cs") = "E:/"` (truthy), `"Engine/Source/X.cpp" = nil` (falsy)
...
`E:\sample\zeqiang_sample_3.4\Source\Client\Intermediate\Build\Android\Client\Development\VulkanRHI\Definitions.VulkanRHI.h`
...
- Completed:
  - `ue.dap.android` now carries default attach context for `<android-package>`, device serial supplied by caller, and the known Android symbol lib path `E:/sample/zeqiang_sample_3.4/Source/Client/Binaries/Android/Client_Symbols_v170300916/Client-arm64/libUE4.so`.
