# hana-alice/nvim 1.12.0 — Deliver qualified SuperUnity batches incrementally

> 日期：2026-09-23
> 类型：Minor（已验证二次批次接入正常 prepare 的阶段性交付）
> Git tag：未创建；尚未获得本版本 tag 授权。
> 状态：阶段实现与真实生成发布已交付；现场冻结批次已触发保护回退，稳定启用留待下一迭代。

## 范围与隔离

用户已明确允许先交付经过验证的可用范围，再继续扩大压缩规模与优化性能。
本版的阶段范围是严格语义准入的 **8→1 二次合并批次**，以及正常 current/hot/full
prepare 对既有 proof store 的自动复用。原始语义 CDB、未合并输入与 shader 覆盖必须完整保留；
缺失或过期 receipt 保留原命令，不得在自动交付路径启动冷资格证明。冻结批次的使用仍须经过
完整输入验证、原生监听与失效回退。

这不是全工程 SuperUnity 性能恢复声明。扩大已证明压缩范围、全工程索引与缓存切换成本，
以及剩余导航和历史 dirty 证据，继续作为后续迭代；不得把阶段结果外推到未测范围。
原始工作站路径、完整环境与实验日志继续留在私有证据目录，不随发布文档输出。

行为契约见 [C++ 语义索引覆盖规格](../openspec/specs/cpp-semantic-index-coverage/spec.md)。
本文件归档 [changelog](changelog.md) 当前全部 Unreleased 条目；下方历史正文保持原样，
其中旧测量、旧结论和当时的待办不自动成为本版本最终验收结论。

## 发布验证

- 本版本全量 required-native 回归 **2115/2115** 通过，零失败、零跳过；改动文件 Lua AST lint、Python AST、严格规格校验与 whitespace 检查通过。
- 真实工程正常生成器与 publisher 两遍通过：14,312 个原生源文件、2,094 条 shader 记录完整；995 个 UBT 包装中的已证明 8 个合成 1 个，保留其余 987 个与 211 条 exact 命令。原始语义 CDB 为 1,206 条，冻结视图为 1,199 条。
- 两遍生成耗时 36.89 / 35.36 s，manifest 与发布耗时 2.90 / 0.11 s；第二遍 `changed=false`，八个产物及 995 个共享包装的字节与 mtime 不变，证明资产未变化，无新冷资格证明。此计时包括整份生成流水线，不能与少量 TU 的索引时间直接相减。
- 当前编辑器已原地载入新实现，窗口、终端、源码监听及历史 dirty 文件保持。正常调度 full 生成发布耗时 40.5 s；冻结客户端曾成功启动，随后因 `input-changed` 失效，保护回退启动原始索引客户端，冻结客户端退出。触发事件的具体文件及原因尚无证据，不能套用此前 LAST_ACCESS 结论。
- 保护回退状态下再次正常调度 full 用时 37.2 s，七个实际生产产物的字节和 mtime 全部不变，原始客户端 ID 保持不变，无重复重启；最终无构建任务或待执行计时器。此检查不表示冻结索引已经稳定启用，也不代替全量 shard/CPU 性能验收。
- 已证明 8→1 的独立样本冷 shard-cache 索引为 61.01 → 17.61 s；热运行 2.10 / 2.07 s，shard 字节与 mtime 不变。首次显式资格证明 281.50 s，缓存证明复用 7.57 s，独立完整激活 12.31 s。OS 缓存与宿主负载未控制，全工程缓存切换及净收益未验收。
- 全工程性能恢复、冻结批次现场稳定启用及后续压缩扩展仍未完成；版本 tag 未创建。本版交付范围为复用选择与正常生成发布实现，不将保护回退视为加速验收成功。

## 归档工作记录

### 2026-09-23 — Deliver already qualified batches through normal prepare

**Task**
- Ship a verified incremental version before broader compression and full-engine optimization, as explicitly requested by the user.

**Implemented**
- `build_clangd_index.py` and `build_full_cdb.py` accept an absolute `--verified-batch-store` only in explicit reuse-only mode; existing defaults, wrapper locations and proof identities remain unchanged.
- `_build.lua` reads a bounded `batch-store.json` adjacent to the project/target semantic CDB and passes the selected store to current/hot/full generation. Invalid selection fails before producing artifacts and releases the writer lease.
- Existing receipt checks, complete original/shader coverage, separate original semantic authority and guarded frozen activation remain mandatory. No new dependency or proof algorithm was introduced.
- Root guidance, the coverage spec, project overview and architecture now distinguish a delivered incremental version from whole-engine performance restoration.

**Validation**
- Real native external-store fixture 2/2, index generation 35/35, shader routing 3/3 and asynchronous subset/dispatch 11/11 passed. Covers noncontiguous accepted groups, cache-only execution, stale/missing fallback, preserved source/shader coverage and unchanged output/receipt/snapshot/wrapper bytes and mtimes.
- Spec consistency: synchronized `cpp-semantic-index-coverage` with the selected-store delivery contract.
- Full required-native regression: 2115/2115 passed, zero failures/skips. Changed-file Lua AST lint, Python AST, strict coverage-spec validation and whitespace checks passed.
- Actual full generator/publisher passed twice with complete native/shader coverage and unchanged repeat bytes/mtimes; live normal full generation published the same accepted batch in 40.5 s. Live frozen activation subsequently invalidated with `input-changed`; fallback retired the frozen client and retained the original client. Event attribution and sustained live acceleration remain follow-up work.
- A repeated normal live full build completed in 37.2 s, preserving all seven production artifact bytes/mtimes and the same fallback client without another restart; final build queue and timers were idle.

**Follow-ups**
- Expand proven compression and measure complete cache-transition/full-engine performance in later iterations; preserve the scope and first-qualification cost of existing measurements.
- Diagnose the current live frozen-input invalidation with event evidence before changing filtering; do not treat successful isolated activation or generator reuse as sustained live indexing acceptance.
