# hana-alice/nvim 1.4.0 — Minor Release

> Version: 1.4.0
> Repo:    https://github.com/hana-alice/nvim-dot-files
> Platform: Windows 11 / macOS host with native IOS target support
> Date:    2026-08-11
> Type:    Minor release (host/target isolation, macOS IOS workflow, compiler-semantic build orchestration)

---

## One-line summary

macOS now builds UE Mac and IOS targets through native host tools, while IOS owns its complete package/device/install/launch policy and Android's existing PowerShell compatibility path remains Windows-only.
...
### 2026-08-11 — 在 macOS 上以独立 IOS target driver 构建、安装并启动应用

**Task**

让 Neovim 在 macOS host 上原生编译 UE 的 Mac/IOS target，并为 IOS 提供打包、真机选择、安装与启动；
保持 Mac、IOS、Android 的 target-specific 行为彼此隔离，不增加 macOS→Android 或 PowerShell 依赖。

**Implemented**

- `lua/utils/platform/` 改为共享基础接口加 host-owned 可选能力：macOS 提供 `xcrun`、`security`、
  `plutil`，Windows 单独提供 `powershell_entry`；缺失能力返回 structured unavailable。
- 新增 `lua/ue/targets/` registry/contract，以及独立的 IOS、Mac、Android、Win64、Linux driver；
  既有 Android PowerShell SO 路径封装在 `android_windows.lua`，不向 macOS 暴露。
- IOS driver 使用原生 `Mac/Build.sh` 和 `RunUAT.sh BuildCookRun`，严格区分 Build 与
  Build/Cook/Stage/Package/Archive；只接受当前 tuple 产出的已签名
  `Binaries/IOS/Payload/<Target>.app`，不把 raw staged app 当作可安装产物。
- `:UESetIOSDevice`、`:UEInstallIOS`、`:UELaunch` 使用 `xcrun devicectl` JSON 和稳定设备 identifier，
  从当前 app 的 `Info.plist` 读取 bundle id；install/launch 校验退出码、设备、bundle id 与结果身份。
- 新增 `:UECompileForNvim`：先验证 clangd 22.1.x，再按 active target 执行原生 build，且只在
  build 成功后进入现有 RSP/CDB/index prepare 流程；`:UEPrepare` 继续保持 prepare-only。
- 补 host/target 隔离、IOS plan/artifact/device/runtime、命令/API/结构测试，并修复若干仅在 macOS
  暴露的测试路径、动态库别名和断点路径可移植性问题；同步 README、tooling、architecture 与 cheatsheet。
...
- 配置有效 Apple Development identity/provisioning，并让一台配对真机进入 connected tunnel 后，执行并保存
  脱敏的 Build → Package → Install → Launch E2E 证据。
- 单独安装仓库要求的 LLVM clangd 22.1.x 后验证真实大型工程的 RSP/CDB 生成与性能。
