# hana-alice/nvim 1.7.0 — Minor Release

> Version: 1.7.0
> Repo:    https://github.com/hana-alice/nvim-dot-files
> Platform: Windows 11 + Neovide GUI (primary); macOS host for Mac/iOS targets
> Date:    2026-08-25
> Type:    Minor release (main-loop headroom, spec authority loop, scan-root discovery, public-mirror privacy, Android SO transports, C++ gd completeness)

---

## One-line summary

This release makes the editor's *responsiveness* an enforced contract rather than a hope: three
measured "continuously stealing the main loop" defects are gone (clangd no longer claims 96% of all
logical cores, language-server stderr is no longer written+flushed per line on the main loop, and the
2s config-tree poll is disabled), the `gr` references path no longer blocks up to five seconds, and
the always-on stall probe now records *attribution* (cpu/gap ratio) so the next investigation starts
from "who held the loop" instead of "something was slow". Alongside that, specs became the
authoritative behaviour contract for every coding agent, scan roots are derived from real UE build
metadata instead of a guessed anchor, the public mirror's privacy gates can no longer be bypassed by
verification skips, Android SO deployment selects its transport by measured device capability, and
C++ `gd` resolves from canonical entity identity to a unique definition.

## Release gate

- Full regression: `nvim --headless -l tests/run.lua` → **1150/1150 passed, 0 failed**.
- OpenSpec: `openspec validate --all` → **39 passed, 0 failed**.
- `openspec/specs/editor-behavior-regression` gained a "main-loop headroom" requirement
  (clangd core reservation / LSP stderr not persisted / no periodic fs polling / no synchronous
  blocking on interactive paths / stall records carry attribution).
- New pitfalls recorded in `docs/CONSTRAINTS.md`: **K52** (the three continuous main-loop taxes,
  including the two methodology traps that made them expensive to find) and **K53** (`vim.system():wait()`
  costs 87ms p50 on this host before doing any work).
- Generic secret scanner (`scripts/check_secrets.sh`) clean; private denylist, ref allowlist and all
  three local privacy hooks (`pre-commit`, `pre-push`, `reference-transaction`) verified installed and
  fail-closed before publication.
- Git tag `v1.7.0` is intentionally pending explicit user confirmation (repo git policy).

## Verified caveats

- The main-loop fixes were verified on an idle session and by direct measurement of each defect; the
  original heavy workload (indexing + Neovide + UBT build simultaneously) was **not** re-run. Use
  `:StallReport` — its new attribution column routes the next report directly.
- Client-side (Neovide) render lag remains outside the probe's visibility by construction.
- A long-running session showed 113 uv timers (104 idle, never closed) vs 9–12 in a fresh instance.
  Confirmed **not** the cause of the 2s stall trains; tracked as a follow-up.

---

## Working log (sliced from docs/changelog.md Unreleased, newest first)

### 2026-08-25 — 拿回主循环余量：clangd 占满核 / LSP stderr 每条 flush / 2s 配置轮询

**Task**

「让 IDE 丝滑起来，任何时候都不能卡」。先查已有探针日志与现有设计，定位并消除持续偷主循环的结构性开销。

**证据（先读日志，后下结论）**

- 日志 scope `[stall]` 累计 **8643 条**（p50 229ms / p90 504ms / p99 1234ms）。
- **8518 / 8643 在 0.3s 内无任何按键** → 与按键动作无关，是后台成因。
- 相邻卡顿间隔最常见值是 **2s**（672 次）；`ft=cpp` 占 6536 条；`mode=n` 占 7771 条。
- **两个互不相干的 nvim 进程按小时计数完全相同**（21T14 都是 325）→ 指向共享磁盘状态 / 宿主资源。

**诊断过程（被证伪的候选比结论更值钱）**

逐一**实测证伪**了全部直觉候选，避开了按直觉改代码：

| 候选 | 实测结果 | 结论 |
|---|---|---|
| 输入路径（按键/滚动） | 活实例 cpp buffer 里每个导航键 p90 < 15ms | 证伪 |
| snacks statuscolumn（50ms 清 cache） | warm 0.006ms、cold/warm 1.2x、整窗 0.15ms | 证伪 |
| 合成 CPU 压力 | 23 个满转 worker：33 文件扫描 1.3→2.9ms，**0 次迟到 tick** | 证伪 |
| 合成磁盘写压力 | 8 个写 worker：1.2x，**0 次迟到 tick** | 证伪 |
| headless 下真实 clangd | 17.4GB RSS / 38 线程，**0 次卡顿** | 证伪（方法论陷阱，见下）|
| uv timer 泄漏 | 活会话 113 timer（104 idle）vs 新实例 9–12 | 疑点，但不是 2s 卡顿成因 |

**两个方法论教训（已写入 K52，比修复本身更可复用）**

1. `--headless` **结构上无法重现 UI 争用**：无渲染线程、无 UI 管道，几乎不需 CPU。本次 6 次
   headless 实验全部报告「主循环很顺」，而用户 GUI 会话正以 30 stalls/min 卡顿。必须用
   `--server` 注入**活的 GUI 实例**测量。
2. `jit.profile` 在空闲时把采样记在最后运行过的栈上（本例 61% 记到 `vim.schedule_wrap`）：
   **它回答「哪段 Lua 跑得多」，不回答「这 250ms 是谁占的」**。归属判定改用 rusage
   **cpu/gap 比值**：≈1 = 本进程内阻塞；≈0 = 被剥夺调度（宿主超订）。

**把诊断能力回浇到常驻探针（否则下次仍会追错）**

上述第 2 条是本次破案的**决定性证据**，但它最初只存在于一个需要手动注入的 `tools/` 脚本里。
而常驻探针（`utils/stall_probe.lua`）只记「卡了多久 + 当时按了什么键」——**下次会话读到同样的
8000 条记录，依然会掉进同一个坑**。所以把归属回浇为常驻能力：

- `utils/stall_probe.lua` 每条记录新增 `verdict` / `cpu_share` / `cpu_ms`，且**同时写进日志**
  （ring 随会话消亡，而跳越会话的日志分析正是发现 2s 节奉与双进程相关的手段）。
- 新增纯函数 `M._verdict(cpu_ms, gap_ms)`：保留 **mixed 中间带**而不强行二分，rusage 不可用时
  诚实返回 `unknown` 而不默认猜一个。
- 新增 `M.summary()`；`:StallReport` 现在**先打聚合**（verdict 分布 + 「有无新鲜按键」对比）
  再列明细——避免靠人眼从上百条里看出「它们都隔 2s 且都没按键」。
- 归属快照必须在 **fast event 里先取**（而非 `vim.schedule` 回调里），CPU 差值才只覆盖 gap
  本身，不包含回调自己等待被调度的时间。
- 开销已实测：`uv.getrusage()` 0.00132ms/次，10Hz 下 **0.013ms/秒**（可忽略）。

一个被自己推翻的测量：首版 change_detection 计时把 `vim.wait(2000)` 包在计时区间内，每次都报
~2082ms（=2000ms 自己的 sleep）；改用 FIFO sentinel 重测后真值是 21ms。**绝不在计时区间内放
固定时长的等待。**

**根因（三个独立的持续性开销，都不是「某个回调很慢」）**

1. **clangd `-j` 只按 RAM 推导** → `floor(94/4)=23`，在 24 逻辑核上占 **96% 的核**，只剩 **1 核**
   给主循环 + Neovide 渲染线程 + 合成器。实测索引时宿主 CPU 6% → 60%，clangd 38 线程。
2. **vim.lsp 把 server 的每条 stderr 当ERROR 同步落盘**（`_transport.lua:36` + `log.lua` 默认
   WARN 门槛使 ERROR 恒通过 + 每条 `write()` 后 `flush()`）→ `lsp.log` 16.6MB / **58803 条，
   全是 clangd 的普通 `Indexed ...` 信息**；0.045ms/条（batched 0.094ms、max 0.63ms），累计约 2.6s
   主循环时间，**恰好在索引时成串爆发**。
3. **lazy.nvim `change_detection` 的 2s 主循环 fs 轮询** → `start(2000,2000)` 同步 `fs_stat` 33 个
   spec 文件（1.2ms p50），命中变更再付 `Plugin.load()`+autocmd **21ms p50**；因为它盯的是
   **共享磁盘状态**，所有 nvim 实例在同一个 2s 窗口一起反应——这就是「两进程计数相同」的成因。

**Implemented**

- 新增 `lua/ue/clangd_jobs.lua`：`M.compute(total_mb, cpus)` 纯函数同时受 RAM 与 CPU 两个预算
  约束，保留 `M.UI_RESERVED_CORES = 4` 个逻辑核给 UI；`M.resolve()` 探测宿主并优先尊重
  `UE_CLANGD_JOBS`。本机效果：**-j=23 → -j=20**。
- `lua/ue.lua` `clangd_cmd` 改为委派 `ue.clangd_jobs.resolve()`；新增 `M.clangd_jobs` 重导出供 spec
  调用。叠加本次抽离后 `ue.lua` **10562 → 10555 行**，**ratchet 下降而非上调**。
- 新增 `lua/config/ui_responsiveness.lua`：`setup_lsp_logging()` 把 vim.lsp 日志置 OFF（实测
  0.045ms → 0.000ms/条）并注册 `:LspLogLevel` 供排查时临时提级/恢复；
  `assert_change_detection_disabled()` 做配置漂移守卫（只记 WARN，不致命）。
- `init.lua` 在 `config.lazy` **之前**调 `config.ui_responsiveness.setup()`，确保首个 language server
  spawn 时日志已关（启动顺序属 C3 承重项，已在注释里说明为何不能往后放）。
- `lua/config/lazy.lua` `change_detection.enabled = false`（无条件，比原来的
  `not health_no_mutate` 严格更强），并写明为何本仓不需要热重载（行为由启动顺序建立，C3）。
- 新增诊断工具（均不默认启用，P5/P6）：`tools/stall_profile.lua`（jit.profile 采样，含其
  局限性说明）、`tools/stall_attribute.lua`（**rusage cpu/gap 归属判定** + uv handle/autocmd/LSP 普查）、
  `tools/stall_repro.lua`（在活实例上回放导航键测每键延迟，默认只跑只读场景不脏用户 buffer）。

**第四个缺陷（由读调用链而非采样发现）——`gr` 同时阻塞两层**

上述三项是「持续偷主循环」；另有一类是「单次卡得很凶」。顺调用链审查发现 `gr`（references）
**在同一个按键上阻塞两次**：`provider.sync_locations` 的 `client:request_sync(..., 5000)`
最多堵 **5 秒**；接着 `ue.gtags_references` 经 `global_lines`→`run_lines` 进入
`vim.system(...):wait()`。

实测出一个关键宿主常数：**本机 `vim.system():wait()` 的空 spawn 底线就是 87ms p50**
（`global -r` 82ms p50 / 293ms max）——Windows 创建进程昂贵，**即使查询一无所获也要付这笔**。
这反过来解释了卡顿分布 p50=229ms：约等于 2–3 次同步 spawn。

- `lua/ue.lua` 新增 `M.gtags_references_async`（复用已有 `global_lines_async`）。
- `lua/utils/lsp_fallback.lua` `M.references` 整条链路改异步：`provider.async_lsp_request`
  → async GTAGS 回退；保持原有 quickfix 标题、回退顺序与「未找到」提示不变。
- `lua/utils/ue_goto/provider.lua` `async_lsp_request` 补上 `includeDeclaration`——sync 版本一直
  在设 ReferenceContext，异步版本原本没设；**不补就会静默改变 `gr` 的返回集**（行为漂移
  比性能回退更难发现，已写入 K53 与回归用例）。

**Pitfalls**

- 新增 **K52**（`docs/CONSTRAINTS.md`）：三连根因 + 完整证伪表 + 两个方法论陷阱。
- 新增 **K53**：Windows 上 `vim.system():wait()` 的空 spawn 底线即 87ms，交互路径禁止同步子进程
  与 `request_sync`；并记下「换异步通道时必须保留 `includeDeclaration`」这个行为陷阱。
- `core_health_spec` 原先断言 `enabled = not health_no_mutate` 字面出现 **2 次**——这是在断言
  「写法」而非「不变量」。已改为逐个检查 `checker` / `change_detection` 各自失效，并接受
  无条件 `false` 为**更强**条件（永远关掉的东西不可能在任何模式下 mutate）。
- `ue.lua` 行数 ratchet 是真门禁：新增 async twin 后一度涨到 10602，通过把 `-j` 策略整体外提到
  `lua/ue/clangd_jobs.lua`（而非上调基线）回到 10562。

**Validation**

- 分范围：`ui_responsiveness` 18/18、`stall_probe` 19/19（新增归属与聚合用例）、
  `core_health` 28/28、`stability` 10/10、`ue_goto_behavior` 8/8、`utils` 49/49。
- **全量回归：`nvim --headless -l tests/run.lua` → 1140/1140 passed, 0 failed**（修复前基线
  1135/1137，两条 FAIL 均由本次改动引入并已处置：ratchet 通过抽离模块下降行数解决，
  core_health 通过修正断言到真实不变量解决）。
- 真实启动复测：`-j=20`（24 核保留 4）、LSP 日志 OFF（2000 条 stderr → 0.0ms、lsp.log 增长
  0 字节）、`reloader.timer` 未 armed、**active 重复 timer 里不再有任何 2000ms 项**。
- 启动耗时未退化：三次 `--startuptime` 均在 240–265ms。
- spec 一致性：已在 `openspec/specs/editor-behavior-regression/spec.md` 新增「主循环余量
  （main-loop headroom）」 requirement（clangd 保留核 / LSP stderr 不落盘 / 禁周期 fs 轮询 /
  交互路径禁同步阻塞 / K40 一般化），并同步 `tests/AGENTS.md` 与
  `docs/testing-regression.md` 的 filter 映射。

**Follow-ups**

- 常驻探针现在能区分 in-process / descheduled，但**仍看不到 Neovide 客户端的渲染延迟**
  （进程外）。若以后出现「体验卡但 `:StallReport` 干净」，那就是 GUI 侧，需另一类探针。
- `tools/stall_{profile,attribute,repro}.lua` 仍无回归覆盖（诊断工具，非运行时路径）；
  其中最关键的归属逻辑已上提到 `stall_probe._verdict` 并被用例锁住。
- 活会话观测到 **113 个 uv timer（104 idle 且未 close）**，而新实例只有 9–12。本次已确认它
  **不是** 2s 卡顿串的成因（空闲 20s 窗口 0 卡顿），但 idle-未close 的 handle 会扣住其闭包。
  待用 `new_timer` 包装 + traceback 在**长时间活会话**上定位所有者（本次短会话复现不出）。
- 本次未能在真实 Neovide 下量化 UI 管道背压（`--embed` 子进程实验在停止 drain 后无法回收
  子进程日志）。若后续仍有残留卡顿，下一步就是直接看 `:StallReport` 的 attribution 行
  （descheduled 占多数即为宿主/GPU 争用）。
...
把本次历史脱敏事故固化为规范与本地门禁：专属关键词匹配只能存在本机，不得为了扫描而再次进入
公开 scanner、spec、fixture 或 Git 历史。

**Implemented**

- 新增 `openspec/specs/public-mirror-privacy/spec.md`，规定专属 denylist 位于 Git directory 或
  worktree 外部、混淆/编码同样禁止、commit/push fail closed，并要求历史清理覆盖全部公开 refs。
- 本地 `.git/private-denylist` 保存真实匹配规则；`.git/hooks/private-denylist-scan` 提供共享扫描逻辑。
- 本地 `pre-commit` / `pre-push` 对所有新增行执行私有扫描，公开 scanner 即使走通用 allowlist 也不能
  绕过本地 denylist；缺失、空文件或无效正则均阻断。
- 本地新增不可被 `--no-verify` 关闭的 `reference-transaction` 门禁，覆盖普通 commit、
  `commit-tree + update-ref`、提交消息、文件路径、非 fast-forward 与 root rewrite 的完整新历史。
- `pre-commit` / `pre-push` 在通用 scanner 缺失时仍执行私有门禁，并将 PASS/BLOCKED 写入仅位于
  Git directory 的审计日志；明确跳过 lint 使用专用 override，不再跳过整组 hooks。
- 公开 remote 的默认 push refspec 与本地 remote-ref allowlist 仅包含批准分支；旧恢复引用从
  `refs/heads/backup/*` 原样迁移到 `refs/private-backup/*`，避免被 `push --all` 带出。

**Pitfalls / Gotchas**

- 将敏感词拆分、编码或拼成正则后上传，仍然是在公开仓库存储可恢复的敏感词表，不是脱敏。
- tracked hook 模板只能描述加载协议，真实 denylist 必须由 `.git/` 或 repo-local config 管理。
- `pre-commit` / `pre-push` 都是可跳过的 client hook；必须用 `reference-transaction` 覆盖本地 ref
  更新，再把 `push --no-verify/--all/--mirror` 设为流程级禁止项。

**Validation**

- 按用户指示未运行项目测试或 Lua lint。
- 实际发布使用正常 `git commit --amend` 与 `git push`；本地 `pre-commit`、
  `reference-transaction`、`pre-push` 隐私门禁均 PASS，结果记录在仅位于 Git directory 的审计日志。
- 直接核对 tracked 文件不包含本地 denylist，公开 remote 只保留批准的两个 heads 且无 tag。

**Follow-ups**

- 其他 clone 必须单独配置本地 denylist；不得从公开仓库分发真实词表。

### 2026-08-24 — Rewrite main as a sanitized public root snapshot

**Task**

重新审计公开远端 `main`，移除当前树和提交历史中的私有项目、公司、设备与工作站身份，避免
历史项目代号、公司域名及相关现场数据继续通过默认分支或祖先提交被搜索和恢复。

**Implemented**

- 将文档、OpenSpec 归档、测试 fixture、DAP probe 与 Android F9 原始证据中的真实项目根、固定
  `Client` 产物布局、设备 serial、个人用户路径和私有 IP 改为稳定的 `SampleGame`、
  `EngineWorktree`、`ANDROID-SERIAL-*` 与 `<PRIVATE_IP>` 公共占位语汇。
- `lua/ue/dap/android.lua` 移除工作站专属 build-root 默认值；未配置
  `dap.android_build_root` 时使用当前 project root，并补充对应行为锁。
- `scripts/check_secrets.sh` 不再携带历史公司/项目专属词表（包括混淆或运行时拼装形式）；公开检测器
  只保留通用凭据与 RFC1918 地址规则，避免检测规则本身成为代码搜索命中。
- `main` 由脱敏后的当前快照生成单一根提交；旧 tip 仅保留在未推送的本地备份 ref。

**Pitfalls / Gotchas**

- 在旧 `main` 上追加 redaction commit 不能移除祖先 blob 和 346 条个人/公司提交元数据命中；必须
  建立无父提交的新根并 force-push。
- 其他远端分支和 tags 仍可能引用旧 DAG。本次只重写 `main`，并重挂当前 iOS feature 分支；不擅自
  删除或改写未授权 refs。

**Validation**

- 默认分支 tracked 内容对历史项目/公司/设备标识的定向扫描：0 命中。
- 全 tracked 文本经 `scripts/check_secrets.sh`：通过；Android F9 evidence JSON：全部可解析；
  `git diff --check`：通过。
- 按用户指示，本次脱敏历史重写不跑 Neovim 回归测试或物理设备验证。

**Follow-ups**

- 审计并单独决定其他远端 branches/tags 的保留、重写或删除策略。

### 2026-08-24 — Re-sanitize the public branch before rewriting its history

**Task**

拉取 `feat/macos-ios-nvim` 最新远端后重新审计公开历史，移除提交元数据与 tracked 内容中残留的
公司身份、真实工程环境和设备标识，再将分支压缩为单个可审计提交。

**Implemented**

- 将文档、OpenSpec 归档、测试 fixture、DAP probe 与 `tools/evidence/android-f9/` 中的真实项目根、
  Android serial、个人用户路径、私有 IP、固定 `Client` 产物布局替换为稳定的 `SampleGame` /
  `EngineWorktree` / `ANDROID-SERIAL-*` 占位语汇；保留调试结论和证据结构。
- `lua/ue/dap/android.lua` 不再把某台工作站的 build root 当默认值；未显式配置
  `dap.android_build_root` 时使用当前 project root，并由 `tests/cases/dap_spec.lua` 锁住。
- `scripts/check_secrets.sh` 仅保留不携带专属关键词的通用凭据与 RFC1918 地址检测；squash commit
  使用公开 noreply 身份，移除分支旧提交中的公司邮箱元数据。

**Pitfalls / Gotchas**

- 未发现私钥、PAT、AWS key 等可直接利用的 secret；实际暴露面是项目路径、设备 serial、私有 IP
  与公司邮箱。只新增“修复提交”不能清掉这些历史元数据，必须 squash 并 force-push。
- 原始 Android F9 JSON 是单行大证据文件；脱敏只替换身份/路径字段，不删除 stop、breakpoint、
  address 与结果结构，避免把历史可复验证据降成口头结论。

**Validation**

- 全 tracked 文本（排除检测器规则本身）经 `scripts/check_secrets.sh`：通过；已知旧标识定向 `rg`：0 命中。
- `tools/evidence/android-f9/*.json` 全部可解析；`git diff HEAD --check`：通过。
- `nvim --headless -l tests/run.lua dap`：72/72 passed；按用户指示本次不再跑全量回归。

**Follow-ups**

- 无；远端更新后以精确 lease 核验单提交拓扑与公开作者身份。
...
- 与 `PrepareIOSQADebug.sh` 的重签流程对齐：无参数命令优先读取标准项目或
  `workspace/Source/SampleGame` 上层 workspace 的 `Saved/IOSQADebug/signing.json`，验证 v1、完整
