# hana-alice/nvim 1.9.0 — 调试、语义与状态隔离

> 日期：2026-09-08
> 类型：Minor（归档自 1.8.0 后累积的 DAP 分层诊断、目标工作流与正确性修复）
> Git tag：待显式授权，本次只提交并推送代码与版本记录。

## 版本摘要

本版本归档 DAP 分层与当前构建符号选择，以及全仓审查中的 15 项缺陷修复。项目交接和异步完成绑定原 owner；索引使用完整编译上下文并在构建成功后发布；编辑交互和干净 CI 环境有真实行为回归。保留原有 target-specific 调试约束，不通过猜测符号或混用其他构建的产物宣称成功。

## 验证与边界

- 本机及隔离 Windows config/data 环境全量均为 **1521/1521，0 failed**。
- 旧 smoke **97/97**；Lua 静态检查 **186 文件通过**；Go 回归、Python AST、核心健康和结构回归通过。
- 本次改动的主规格已同步；完成记录归档至 `openspec/changes/archive/2026-09-08-close-reviewed-config-defects/`。
- 归档后提交前全量再次 **1521/1521**；全仓主规格严格验证 **40/40**；最终 staged 的私有 denylist 与通用敏感信息扫描通过，门禁结果仅记入本地 Git 审计日志。
- 未触发远程 CI；macOS/Linux、真机调试与大 UE 工程重索引性能未在本轮复测。
- 旧 Neovim 实例需重启加载新锁协议；损坏 owner 保留证据并 fail closed，持续不可写磁盘不会被报告为已保存。
- 隐私 hooks 全程保留，公开 scanner 不再提供绕过指引；真实 denylist 与原始审计材料仅存 Git directory。

## 工作记录（从 Unreleased 归档，历史验证范围保留原义）

### 2026-09-08 — 修复全仓审查的状态隔离、索引语义与交互缺陷

**Task**

处理全仓审查确认的 15 项问题，并验证和修复两个语义缓存疑点；保留此前未提交的 Android/DAP 改动。

**Implemented**

- `lua/ue/dap/_common.lua` 补齐桌面 binary prompt 的 fs 依赖；`_persist_bp.lua` 在项目交接时保存、移除旧 bucket 管理的断点并恢复新 bucket，活跃 DAP 会话期间延迟交接。
- `lua/ue/file_lock.lua` 以完整的唯一 token owner 目录发布 lease，回收只 unlink 已观察 owner 并非递归 rmdir；未知损坏 owner 保留证据并明确诊断，不以探测权限失败推断进程死亡。
- `lua/utils/ue_watch.lua` 隔离每个 project 的状态与事件 generation；持久化辅助逻辑提取到 `lua/utils/dirty_save.lua`，旧 owner 保存绑定原路径，锁忙及 open/write/close/rename 失败统一有界退避，成功后取消旧重试。
- `lua/ue/project_state.lua` 和 `ue.lua` runtime updater 接受捕获的 project 地址；iOS install/launch 完成回调写回原 bucket，不改变用户当前选择。
- `tools/replace_i_with_rsp.py` 仅展开 active argv 显式 response；`build_hot_super_unity_cdb.py` 比较完整语义 argv，不匹配使用 exact fallback；`inject_definitions_to_cdb.py` 与 Full/current 入口移除 Editor 路径猜测，保留并验证显式 Definitions/PCH，不发布缺失输入的 ready marker。
- `tools/cindex-uefilter/main.go` 的 reset/add 均先写暂存再替换正式索引，Windows 待 writer 子进程释放句柄后发布；失败保留原索引，并正确转发 stdin。已重新构建安装本机原有 cindex-uefilter。
- `lua/utils/code_search/init.lua` 可用性检查只读正式路径，以有界格式头尾/offset 检查替代体积猜测，不提升 writer 暂存文件。
- `lua/utils/ue_goto/semantic_transaction.lua` 按 client encoding 转换 snapshot 光标，按精确位置/范围判断同一目标；header callback 的 lineage 与直接跳转均先检查请求和 generation。
- sidecar 使用 compiler-authored inclusion 集合及磁盘文件签名，使已保存 source/header 变化失效 warm TU 和 destination cache；检查留在 sidecar，不扫描整个项目。
- `lua/config/keymaps.lua` 使用当前 Visual 范围并修正 replacement 插入位；`lua/plugins/snacks.lua` 删除通用光标回拉；`lua/trouble/sources/ue_sidebar.lua` 消费 NUL porcelain 原始路径及 rename/copy 记录。
- 旧 smoke 对齐真实 host matrix 与 libuv 环境数组；CI 恢复 lock 中既有插件与 parser，并接入全量与 Go 回归；新增独立交互、归属及 CI 回归。
- `lua/plugins/toolchain.lua` 清除继承的 Mason 自动安装计划并保留手动能力；`lua/config/lazy.lua` 在只读 health 缺少 lazy.nvim 时失败，不再尝试 clone。
- 同步 8 项相关行为规格，以及 C++ source identity 的本地规则/架构摘要，清除旧 libclang-only 和历史 adapter 注释漂移。

**Pitfalls / Gotchas**

- 旧 Neovim 进程仍持有旧 Lua 代码；共用 cache 的实例需要重启后才能全部使用新锁协议。新实现不能阻止旧客户端继续递归删除锁。
- lease 发布前崩溃可能留下不占正式锁的 `.pending.<token>` 小目录；损坏且无可验证 owner 的非空锁按 fail closed 处理。
- dirty 保存持续失败超过有界重试时会告警，不能保证不可写磁盘上的落盘；严格 unity 证明可能增加 exact TU 比例。
- 大 UE 工程的重索引性能、PCH ABI、真机 DAP 与原 Neovide mouse-release 现象未在本轮重新测量；不以 headless 结果替代这些证据。

**Validation**

- 新增用例已先复现红灯再修复；包括真实文件系统锁交错、Git 特殊文件名、Go reset 读写隔离，以及真实 Clang/sidecar 的保存文件失效。
- 本机 `nvim --headless -l tests/run.lua`：1521/1521，0 failed（对子进程设置真实 LLVM `UE_CLANGD`，包含 Clang/sidecar fixture）；新建隔离 Windows config/data 环境的同一全量入口也为 1521/1521，退出码均为 0。
- `scripts/headless_smoke.lua`：97/97；`scripts/lint_no_bare_globals.lua`：186 个 Lua 文件通过；`scripts/nvim_core_health.lua --json`：PASS（真实启动、编辑、parser、rg/csearch、clangd 和 target plans；live workspace/device 未运行）。
- `tools/cindex-uefilter`：`go test -p 1 -count=1 ./...` 通过；5 个改动 Python 文件 AST parse 通过；`git diff --check` 通过。
- 最终文档/规则结构回归：`structure` 75/75，通过。
- Windows 隔离 CI bootstrap 已真实恢复锁定插件、编译并加载 c/cpp/hlsl parser，core_health 范围 28/28；远程 workflow 未触发，macOS/Linux 执行仍待远程 CI。
- Spec 一致性：同步 `multi-instance-state-isolation`、`ue-code-search`、`cpp-contextual-definition-navigation`、`cpp-semantic-index-coverage`、`keymap-command-regression`、`editor-behavior-regression`、`config-regression-suite`、`nvim-core-functionality-audit`；8 项严格验证全部通过。

**Follow-ups**

- 不自动提交、推送或重启用户正在编辑的实例；完成后的运行环境边界见上述说明。

### 2026-09-04 — 当前配置未 strip 产物直接作为 Android DAP 符号源（K66–K69）

**Task**

继续上一会话的 K65 真机验证。Test 配置其实已有未 strip UBT 产物；此前工具只看
`*_Symbols_v*`，把可用符号漏掉并错误要求构建/安装另一个配置。

**Implemented**

- `lua/ue/dap/_android_symbols.lua` 按 ELF64 little-endian section table 验证真实非空
  `.debug_info`/`.zdebug_info`，并从 `.dynamic` 的 `DT_SONAME` 读取 runtime module identity。
  不再靠头部字符串扫描；大体积 `.dynstr` 只读取 SONAME 所在的有界切片。
- 新增 `lua/ue/target_identity.lua`，把原先 `ue.lua` 内的 Configuration 拆分、Target.cs 发现与
  Target 选择抽成 build/DAP 共用的 headless API（首轮 inline 使 `ue.lua` 到 10575 行并触发
  ratchet；抽取后低于冻结上限 10562）。`lua/ue/dap.lua` 消费同一 resolver；build 的
  `UE_TARGET_CONFIGURATION` override 与项目默认选择仍经 `selected_target_configuration` 进入 DAP，
  注册到 nvim-dap 的 configuration 也回到同一 façade，不再直调 `android.attach({})` 绕过富化。
  `lua/ue/targets/android.lua` 导出配置限定的 symbol artifact resolver，支持 Development
  `<Target>-arm64.so`，同时保持部署与 mismatched receipt 下 generic short-name 的严格拒绝。
- `lua/ue/dap/android.lua` 自动优先选择当前 Target/Configuration 的未 strip 产物，并把 host
  symbol basename、runtime `DT_SONAME`、packageInfo versionCode 分开保存。普通 attach/launch
  不再从 `_last_session.symbol_lib` 绕过重选；已有 packageInfo 却无唯一同版本候选时不再按 mtime
  猜另一个版本。K69 进一步把 reattach 快照成功条件从「有 PID」收紧为 DAP `attach` response
  明确成功，L2 拒绝/handshake failure/仅有 initialized event 都不再污染 `_last_session`。
- normal attach 与 wait-mode late rebase 现在都以 runtime SONAME 查 `/proc/<pid>/maps`，但
  `target create` / `target modules load --file` / 致命信号符号断点仍指向 host 符号模块；K37 slide
  保持 load-bearing。
- 真机运行顺带抓到拆 owner 后的 namespace 漏洞（K67）：transport 内部裸调用注入依赖/helper
  会报 nil，且 `pcall` 会静默吞掉 kill/rm 缺席；全部改为 `deps.*` / `M.*`，并补实际编排行为测。
  四个拆分模块的 `bind()` 现在真实拒绝未知键（旧 `or key ~= nil` 永远为 true）；
  `_android_engine.lua` 的 `vim.deps.log.levels` typo 同步改为 `vim.log.levels`。
- 同步 `openspec/specs/android-dap-attach/spec.md`、`docs/CONSTRAINTS.md` 与
  `lessons/README.md`；新增 ELF、直接产物优先级、Target≠Project、host/runtime basename、
  stale last-session、弱匹配拒猜、transport 编排与 target-owner 复用回归。

**Pitfalls / Gotchas**

- ELF section header 在文件尾部；只读前几 KB 搜 `.debug_info` 会假阴性。反过来，只在 section
  name string table 搜字面量又会把未被任何 section 引用的残留名字判成假阳性。
- 真实 UE `.dynstr` 可超过 70 MB；为了一个 SONAME 读完整表没有必要，先从 `DT_SONAME` 取 offset，
  再只读有界 C string。
- host 文件名 `<Target>-Android-<Cfg>-arm64.so` 与设备 maps 的 `libUE4.so` 不是同一 identity。
  直接复用 host basename 查 maps 会让显式 slide 消失，必须用 `DT_SONAME` 建桥。
- PID 在 L2 gate 与异步 attach response **之前**就已捕获；失败 adapter 甚至也可能先发
  `initialized`。只有 attach response success 能证明可持久化 reattach session（K69）。
- 首轮全量回归把直接塞进 `ue.lua` 的 target resolver 以 line-count ratchet 拦下；正确收敛是把
  原有 build identity 逻辑抽成共享模块，而不是上调上限或删无关注释凑行数。
- 真机当前设备还暴露 K68：app-uid server 可执行且 LISTEN，但 forwarded GDB handshake 不回包；
  同设备同 binary 的 shell-uid control 立即 ACK。K56 已证明 shell uid 无权 ptrace app，所以不能
  以 shell fallback 假装修好。

**Validation**

- 探针 report-first：`ue_probes.json` 只有历史/已过期记录，无本轮新鲜失败证据；本工作继续处置
  同一 Android DAP L4 链路。
- 真实工程只读：自动选择当前 Test 配置未 strip artifact；`.debug_info=true`、
  `DT_SONAME=libUE4.so`；staging 后 `:UEDAPPreflight` L0–L4 全绿。
- 真机 attach 部分通过：lldb-dap `target create` 成功解析该 artifact 的 symbol table/DWARF，
  attachCommands 正确生成 `maps(libUE4.so) → load(host artifact, slide)`；最终 attach 被 K68 的
  forwarded handshake timeout 阻断，**未宣称端到端成功**。A/B evidence：app-uid server
  process+LISTEN 但 GDB packet timeout；同 binary shell-uid server 立即返回 ACK。现场 server/forward
  已全部清理。
- `dap_failure_layer` → **100/100**；`dap_spec` → **113/113**；`ue_target_drivers` → **48/48**；
  `stability` → **10/10**。
- 首轮全量唯一红灯为 `ue.lua` line-count ratchet（10575 > 10562）；抽取共享
  `ue.target_identity` 后重跑全量，fresh-review 修复后最终 → **1481/1481 passed, 0 failed**。
- `openspec validate --all --strict` → **40/40**；`scripts/lint_no_bare_globals.lua`
  （9 个改动 Lua 文件）→ OK；`git diff --check` → clean。
- spec 一致性处置：**已同步** `android-dap-attach` 的符号优先级、fail-closed 规则、
  Target/Configuration 来源、DT_SONAME 映射与 normal/late rebase 行为；同步
  `android-dap-handshake-diagnostics` 的 app-uid vs shell-control A/B 判定边界。
- fresh-context review 的 5 条 finding 已全部处置：env/default configuration parity、失败 attach
  快照、Development short name、registered DAP entrypoint、bind typo guard。

**Follow-ups**

- K68 是当前设备的 external target-policy blocker：runtime 自动门禁须先立 change 修改现有
  diagnostic-only 契约，再在启动 lldb-dap 前加入 app-uid forwarded handshake probe（必要时带
  same-binary shell control），把 ACK/timeout 作为分层 evidence；在此之前不得仅凭通用 handshake
  错误猜层，也不得回退 K56 已证伪的 shell-uid attach。
- 设备侧已安装 native lib build-id 仍未自动提取；当前 L4 的 versionCode 相等仍只代表 weak match。
...
- 此 follow-up 已由 K66 收敛：artifact 解析复用 target owner；部署继续要求 `<Target>.target`
  tuple 完整匹配，DAP 则可消费文件名携带完整 tuple 的配置限定符号产物。
