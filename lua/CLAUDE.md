...
> 父级：`../CLAUDE.md`（根强制入口：SESSION START + Definition of Done）。
> 本文件是 `lua/` 下所有代码的**父级规则**；子目录 `CLAUDE.md` 只写相对本文件的**增量**。
> 某目录无 `CLAUDE.md` 时，适用最近祖先目录的规则（回落语义）。
...
## 改动后（Definition of Done，权威见根 CLAUDE.md）

改完按范围跑回归（映射见 `../tests/CLAUDE.md`）+ 记 `../docs/changelog.md`。

## 先读

`../docs/CONSTRAINTS.md`、`../docs/architecture/overview.md`、对应子目录 `CLAUDE.md`。
