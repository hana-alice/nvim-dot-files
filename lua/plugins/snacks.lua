...
return {
  {
    ...
    opts = function(_, opts)
      ...
      if not has_open_folder then
        table.insert(opts.dashboard.preset.keys, math.min(2, #opts.dashboard.preset.keys + 1), {
          icon = " ",
          key = "o",
          desc = "Open Folder",
          enabled = function()
            local platform = require("utils.platform")
            return vim.g.neovide and platform.id == "macos"
          end,
          action = function()
            require("config.neovide").open_folder()
          end,
        })
      end

      -- Colored pixel header: 每个字符是一个 ▀ 半块, fg=上像素 bg=下像素.
      -- See lua/dashboard_pix.lua (auto-generated by Temp/img2pixblock.py).
      do
        local ok, pix = pcall(require, "dashboard_pix")
        if ok and pix and pix.combos then
          -- Define all combo highlight groups (fg+bg) once.
          for _, c in ipairs(pix.combos) do
            vim.api.nvim_set_hl(0, c.name, { fg = c.fg, bg = c.bg, default = true })
          end
          -- Drop default header (would render text-only golden ASCII art);
          -- we'll inject pixel rows as custom dashboard items below.
          opts.dashboard.preset.header = ""

          -- Inject pixel rows as a custom section + title big-text.
          -- Strategy: prepend before keys. snacks dashboard supports
          -- `sections = {item1, item2, ...}` where each item with `text =`
          -- becomes one rendered line.
          local pix_section = { padding = 1, align = "center" }
          for _, row in ipairs(pix.lines) do
            -- row is array of {str, hl_name}; convert to snacks Text[] form.
            local text = {}
            for _, run in ipairs(row) do
              text[#text + 1] = { run[1], hl = run[2] }
            end
            pix_section[#pix_section + 1] = { text = text, align = "center" }
          end
          -- Big title under the portrait.
          local title_section = {
            padding = { 1, 1 },
            { text = { { "UE Developer IDE", hl = "SnacksDashboardHeader" } }, align = "center" },
          }
