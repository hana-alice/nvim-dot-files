...
local tabs = {
  ...
  -- ── Tab 6: Tools ──
  {
    ...
    sections = {
      ...
      {
        title = "Git — Inline (gitsigns)",
        mappings = {
          { "]h / [h",        "Next / prev hunk" },
          { "<leader>gb",     "Blame line popup (LazyVim)" },
          { "<leader>gB",     "Full-file blame view (fugitive)" },
          { "<leader>gd",     "Preview diff hunk (LazyVim)" },
          { "<leader>uG",     "Toggle git signs / inline blame" },
          { "(EOL virt text)","author · time · summary, 200ms" },
        },
      },
      {
        title = "Git — Diffview (commit / range / file)",
        mappings = {
          { "<leader>gv",     "Working tree changes" },
          { "<leader>gV",     "Close diffview" },
          { "<leader>gm",     "This file history" },
          { "<leader>gM",     "Branch history (commit picker)" },
          { "<leader>gv (v)", "Selected lines history" },
          { "<leader>gr",     "Arbitrary range (prompt)" },
          { "<leader>gk",     "Single commit (prompt hash)" },
        },
      },
      {
        title = "Inside diffview",
        mappings = {
          { "<tab> / <s-tab>",   "Next / prev file" },
          { "j / k (panel)",     "Next / prev entry no open" },
          { "]c / [c",           "Next/prev hunk (within file)" },
          { "]h / [h",           "Next/prev hunk (cross file)" },
          { "]x / [x",           "Next/prev merge conflict" },
          { "<cr>",              "Open file / commit" },
          { "s (file panel)",    "Stage / unstage file" },
          { "y (history panel)", "Yank commit hash" },
          { "q",                 "Close diffview" },
        },
      },
      {
        title = "Git — Search & history (advanced_git_search)",
        mappings = {
          { "<leader>gh",  "Search commits by content" },
          { "<leader>gH",  "Search commits by content (this file)" },
          { "<leader>gx",  "Diff this file vs branch" },
          { "<leader>gX",  "Diff this file vs commit" },
          { "<leader>gC",  "Checkout from reflog" },
          { "<leader>gA",  "Advanced search palette" },
        },
      },
      {
        title = "Git — Pickers / Neogit / Fugitive",
        mappings = {
          { "<leader>gs",     "Status picker (snacks)" },
          { "<leader>gc",     "Commits picker (snacks)" },
          { "<leader>fg",     "Git files (UE-aware)" },
          { "<leader>gn",     "Neogit (commit/push/pull/stash/branch)" },
          { "<leader>g0",     "Open :0 (staged) version of file" },
          { "<leader>gl",     "This file commits → quickfix" },
          { "<leader>gL",     "All commits → quickfix" },
          { "(no lazygit)",   "This config uses Neogit + diffview only" },
        },
      },
    },
  },
    ...
    sections = {
      {
        ...
        mappings = {
          ...
          { ":UECompileForNvim", "Compatibility: build then normal UEPrepare" },
          { ":UEBuildIOS",   "Build IOS target only" },
          { ":UEIOSSetup",   "Optional: rerun IOS signing/private-key/device setup" },
          { ":UESetIOSSigningCertificate", "Import prepared or select project IOS signing identity" },
      ...
      },
      {
        ...
        mappings = {
          ...
          { ":UESetIOSDevice", "Select IOS device with live discovery/recovery progress" },
          { ":UEInstallIOS", "Install IOS app with live signing/upload/device progress" },
      ...
      },
      {
        ...
        mappings = {
          { "1/2 :UESetProject",  "Bind project + engine (either order)" },
          { "1/2 :UESetPlatform", "Select target (either order)" },
