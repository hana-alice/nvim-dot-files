...
-- Used by the non-C++ compatibility path to build LSP requests and detect
-- cursor contexts that should not be sent to a language server.
...
--
--   Used only by the non-C++ compatibility cache/fallback path. C++ gd does
--   not use receiver text as semantic identity.
--
...
--
-- Compatibility-path goal: stop before asking an LSP, when the symbol
...
--   Non-C++ compatibility detector for "LSP cannot meaningfully resolve a definition
