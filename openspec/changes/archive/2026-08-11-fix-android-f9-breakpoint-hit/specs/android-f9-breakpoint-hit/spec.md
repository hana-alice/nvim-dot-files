...
系统 SHALL 在实现 Android F9 修复前建立当前代码行为图，并保证每个设计决定都有代码路径、
运行行为和验证证据支撑。

#### Scenario: apply 前代码行为审计
- **WHEN** 开始实现本 change
- **THEN** 实现者 SHALL 先记录 F9 从 keymap 到 `_persist_bp`、DAP listener、
  `ue.dap.android` attach config、LLDB command、stop event 的实际路径
...
系统 SHALL 让 Android 断点植入逻辑只有一个运行时 owner，避免重复 preseed 或插入顺序不一致。

#### Scenario: attachCommands 由 Android 模块生成
- **WHEN** 构建 UE Android attach config
- **THEN** 断点 preseed 的收集、命令生成和插入顺序 SHALL 由 `lua/ue/dap/android.lua` 负责
- **AND** `lua/ue/dap.lua` MUST NOT 另行向 Android attachCommands 注入重复 breakpoint 命令
- **AND** 断点命令 SHALL 位于 symbol target、platform connect、process attach、signal disposition、
  ASLR rebase 之后

#### Scenario: 会话中 F9 不静默重连
- **WHEN** 用户在已 attach 的 Android 会话中按 F9 新增或删除断点
- **THEN** 系统 SHALL 要么即时安全下发并验证 LLDB resolved
- **OR** 明确提示需要用户触发 reattach 才能生效
- **AND** MUST NOT 静默 detach/reattach 来伪装即时断点生效

### Requirement: F9 断点端到端命中
系统 SHALL 让 UE Android F9 file:line 断点从编辑器设置到真机运行时命中形成可验证闭环。
...
- **THEN** 系统 SHALL 要么安全地下发并验证该断点 resolved
- **OR** 明确提示该断点需要 reattach 后生效
- **AND** MUST NOT 在未下发或未验证 resolved 时报告误导性的成功状态

### Requirement: 断点成功判据包含 LLDB 证据
系统 SHALL 用 LLDB 层证据决定 Android F9 断点是否成功，而不是只依赖 UI 状态或合成响应。
...
- **AND** SHALL 指明是需要 reattach、路径未匹配、符号未加载、ASLR 未校正还是 LLDB 命令失败
...
- **THEN** 日志 SHALL 包含 nvim 断点表、attachCommands 或即时命令、DAP `setBreakpoints` 响应、
  LLDB `breakpoint list`、adapter exit code、stop event 与 selected frame
- **AND** 日志 SHALL 使用 fresh pid / fresh port，不复用旧会话结论

#### Scenario: 验证失败可定位层级
- **WHEN** F9 未命中
- **THEN** 诊断输出 SHALL 能区分失败发生在未下发、未 resolved、resolved 但未命中、
  已命中但 UI 未跳转这四类之一
