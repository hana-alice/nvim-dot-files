...
系统 SHALL 通过 lldb-dap 的结构化连接路径建立连接，MUST NOT 把
...
- **THEN** 用结构化键（`gdb-remote-port`/`gdb-remote-hostname` / `platformName` 或等价 SBAPI 路径）建立连接
...
系统 SHALL 让 Android file:line 断点真实下发并 resolve，`verified` 反映真实结果，
MUST NOT 无条件返回固定值。
