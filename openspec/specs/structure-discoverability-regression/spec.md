...
把「规则与知识就地可发现」变成可执行的回归守护项。回归套件验证主要目录的本地规则内容源、知识库结构完整性、关键内部链接不悬空、spec 与规则文档的引用不悬空、capability 覆盖映射不腐烂，以及回归/changelog/milestone/spec 一致性政策在文件中可被发现，防止文档腐烂或漂移。
