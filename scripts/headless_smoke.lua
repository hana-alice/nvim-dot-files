...
check("ue.setup() registers Android only on supported hosts", function()
  local p = require("ue.dap.platforms")
  local targets = require("ue.targets")
  local host = require("utils.platform").driver()
  assert((type(p.attach_handler("android")) == "function") == targets.supports("Android", "dap_attach", host))
  assert((type(p.launch_handler("android")) == "function") == targets.supports("Android", "dap_launch", host))
end)
...
check("Phase H: ue.setup() respects the host-target DAP matrix", function()
  local p = require("ue.dap.platforms")
  local targets = require("ue.targets")
  local host = require("utils.platform").driver()
  for _, id in ipairs({ "win64", "mac", "linux", "ios" }) do
    assert((type(p.attach_handler(id)) == "function") == targets.supports(id, "dap_attach", host), id .. " attach matrix mismatch")
    assert((type(p.launch_handler(id)) == "function") == targets.supports(id, "dap_launch", host), id .. " launch matrix mismatch")
  end
end)
...
check("Phase I: CDB defaults retain compiler include search paths", function()
  local steps = require("ue.config").get("cdb.steps")
  assert(#steps == 5)
  assert(not vim.tbl_contains(steps, "prune_include_dirs.py"))
end)
...
check("Phase I: ue.config.dap lldb-dap env defaults are sanitized", function()
  ...
  assert(vim.islist(env), "uv.spawn env must be an array")
  assert(vim.tbl_contains(env, "PYTHONHOME="))
  assert(vim.tbl_contains(env, "PYTHONPATH="))
end)
check("Phase I: ue.config.dap lldb-dap env override", function()
  ...
  cfg.reset_for_test()
  assert(vim.islist(env), "uv.spawn env must be an array")
  assert(vim.tbl_contains(env, "PYTHONHOME=/pyhome"))
  assert(vim.tbl_contains(env, "PYTHONPATH=/pypath"))
end)
