...
t.describe("probe: 持久化往返", function()
  ...
  t.it("切换测试路径会取消旧路径的防抖 save timer", function()
    fresh(function(dir)
      probe.record("timer", "pending")
      t.assert_true(probe._has_pending_save_for_test(), "record 后应有待写 timer")
      probe._set_path_for_test(dir .. "/next.json")
      t.assert_false(probe._has_pending_save_for_test(), "路径切换必须关闭旧 timer")
      vim.wait(2100)
      t.assert_eq(vim.fn.filereadable(dir .. "/next.json"), 0,
        "旧 payload 不得延迟写入新测试路径")
    end)
  end)
end)
