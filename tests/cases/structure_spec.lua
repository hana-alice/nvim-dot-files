...
t.describe("structure: 主要目录本地规则存在", function()
  for _, d in ipairs(MAJOR_DIRS) do
    t.it(d .. "/CLAUDE.md 存在", function()
      ...
      t.assert_true(exists(d .. "/CLAUDE.md"), "缺少本地规则: " .. d .. "/CLAUDE.md")
    end)
  end
end)
...
t.describe("structure: 强制入口与政策可发现", function()
  local root = read("CLAUDE.md") or ""
  local constraints = read("docs/CONSTRAINTS.md") or ""
  local tests_rules = read("tests/CLAUDE.md") or ""

  t.it("根 CLAUDE.md 含 SESSION START 协议块", function()
    t.assert_contains(root, "SESSION START")
  end)
  ...
  t.it("根 CLAUDE.md 引用 changelog 与 milestone", function()
    t.assert_contains(root, "changelog.md")
    t.assert_contains(root, "milestone")
  end)
end)
