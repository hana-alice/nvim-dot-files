...
t.describe("ue.targets registry and contract", function()
  ...
  t.it("keeps iOS DAP artifact and tool policy in the iOS target owner", function()
    local ios = targets.must_get("IOS")
    local artifacts = ios.dap_artifacts({ project_dir = "/Project", target = "SampleGame" })
    t.assert_eq(artifacts.binary, "/Project/Binaries/IOS/SampleGame")
    t.assert_eq(artifacts.dsym, "/Project/Binaries/IOS/SampleGame.dSYM")
    t.assert_eq(ios.dap_tool_names("coredevice").xcrun, "xcrun")
    t.assert_eq(ios.dap_tool_names("legacy-mobiledevice").ideviceinfo, "ideviceinfo")
    t.assert_nil(ios.dap_tool_names("unknown"))
  end)
