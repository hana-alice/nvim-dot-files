...
def main():
    ...
    # Group source CDB entries by module to find representative
    mod_to_template = {}
    for e in cdb:
        mod = get_module_from_filepath(e.get('file', ''))
        if mod and mod not in mod_to_template:
            mod_to_template[mod] = e
    print(f'Modules in source CDB: {len(mod_to_template)}')
    ...
    for mod_dir in sorted(os.listdir(dev_root_local)):
        ...
        # Find template entry from source CDB for this module
        template = mod_to_template.get(mod_dir)
        if not template:
        ...
        for unity_local in unity_files:
            ...
            # Copy template args, swap file
            new_args = list(template.get('arguments', []))
