"""
Definitive test: Can HW BPs fire on threads OTHER than the main attach thread?
1. Attach → get main thread PC, set HW BP at main PC+4 → should fire (baseline)
2. Find GameThread → get GameThread PC → set HW BP at GameThread PC+4 → does it fire?
If #2 fails, HW BPs are per-thread (not propagated to all threads).
"""
import json
import select
import socket
...
class DAPClient:
    ...
    def eval_repl(self, cmd, fid=None, timeout=15):
        ...
        return result, "\n".join(outputs)
    def get_pc(self, fid):
        """Get PC from expression evaluation."""
        result, output = self.eval_repl("?$pc", fid)
        for part in (result + " " + output).split():
            if part.startswith("0x"):
                try: return int(part, 16)
                except: pass
        return None
...
def main():
    ...
    try:
        ...
        p(f"\nAttached. main_tid={main_tid}")

        # Get threads to find GameThread
        th_seq = client.request("threads")
        msgs, th_resp = client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == th_seq, timeout=15)
        threads = (th_resp.get("body") or {}).get("threads", []) if th_resp else []
        game_threads = [t for t in threads if "GameThread" in (t.get("name") or "") and str(main_tid) not in t.get("name", "")]
        p(f"  GameThreads: {[(t['id'], t.get('name','')[:30]) for t in game_threads[:3]]}")

        if not game_threads:
            p("ERROR: no GameThread found")
            return

        gt = game_threads[0]
        gt_tid = gt["id"]
        p(f"  Using GameThread tid={gt_tid}")

        # Get GameThread frame and PC
        st_seq = client.request("stackTrace", {"threadId": gt_tid, "startFrame": 0, "levels": 1})
        ...
        if st_resp:
            ...
            if frs:
                gt_fid = frs[0]["id"]
                gt_frame_name = frs[0].get("name", "?")

        gt_pc = client.get_pc(gt_fid)
        p(f"  GameThread PC = 0x{gt_pc:x} ({gt_frame_name})" if gt_pc else "  GameThread PC = unknown")

        if not gt_pc:
            p("ERROR: can't read GameThread PC")
            return

        gt_pc4 = gt_pc + 4
        p(f"  GameThread PC+4 = 0x{gt_pc4:x}")

        # Also get main thread frame for comparison
        st_seq = client.request("stackTrace", {"threadId": main_tid, "startFrame": 0, "levels": 1})
        msgs, st_resp = client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == st_seq, timeout=15)
        main_fid = None
        if st_resp:
            frs = (st_resp.get("body") or {}).get("stackFrames") or []
            if frs: main_fid = frs[0]["id"]

        # =========================================
        # TEST: Set HW BP at GameThread's PC+4
        # =========================================
        p(f"\n=== TEST: HW BP at GameThread PC+4 (0x{gt_pc4:x}) ===")
        result, output = client.eval_repl(f"breakpoint set -H -a 0x{gt_pc4:x}", main_fid or gt_fid)
        p(f"  {(output+result).strip()[:200]}")
        ...
        if hit:
            ...
            hit_tid = body.get("threadId")
            p(f"\n*** HW BP on GameThread PC+4 FIRED! threadId={hit_tid} ***")
            if hit_tid == gt_tid:
                p("  Hit on GameThread itself — HW BPs DO work on non-main threads!")
                p("  The issue is specific to function/file+line BP resolution.")
            else:
                p(f"  Hit on different thread ({hit_tid}) — address collision, but HW BPs are propagated.")
            p("\nCONCLUSION: HW BPs propagate to all threads. Issue is in BP address resolution.")
        else:
            p("\n*** HW BP on GameThread PC+4 DID NOT FIRE ***")
            p("CONCLUSION: HW BPs are NOT propagated to non-main threads.")
            p("This is a fundamental LLDB limitation on this platform.")
            p("Solution: use SOFTWARE breakpoints (need to fix error:9 / PTRACE_POKETEXT).")

            # Pause and check BP state
            ...
            if stop2:
                ...
                msgs, st2 = client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == st_seq, timeout=10)
                fid2 = None
                if st2:
                    frs = (st2.get("body") or {}).get("stackFrames") or []
                    if frs: fid2 = frs[0]["id"]
                result, output = client.eval_repl("breakpoint list -v", fid2)
                p(f"  BP state: {(output+result).strip()[:300]}")

                # Extra: try SOFTWARE BP at a known-good address to check if PTRACE_POKETEXT works
                p("\n=== Extra: Testing SOFTWARE BP at GameThread PC+8 ===")
                gt_pc8 = gt_pc + 8
                result, output = client.eval_repl(f"breakpoint delete", fid2)
                result, output = client.eval_repl(f"breakpoint set -a 0x{gt_pc8:x}", fid2)
                p(f"  SW BP: {(output+result).strip()[:200]}")

                cont_seq2 = client.request("continue", {"threadId": tid2})
                client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == cont_seq2, timeout=10)
                msgs, hit2 = client.collect_until(
                    lambda m: m.get("type") == "event" and m.get("event") == "stopped"
                              and (m.get("body") or {}).get("reason") == "breakpoint",
                    timeout=10)
                if hit2:
                    p("  *** SOFTWARE BP FIRED! PTRACE_POKETEXT works at correct addresses! ***")
                else:
                    p("  SOFTWARE BP also didn't fire. PTRACE_POKETEXT broken on this platform.")
