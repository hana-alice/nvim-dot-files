"""
Test: do HW BPs persist after continue? Set BP on FEngineLoop::Tick (called every frame).
If this doesn't fire, HW debug registers are being cleared.
"""
import json
import re
import select
...
def main():
    ...
    try:
        ...
        # =====================================================
        # TEST 1: Software BP on FEngineLoop::Tick (no -H flag)
        # =====================================================
        p("\n=== TEST 1: SOFTWARE BP on FEngineLoop::Tick ===")
        result, output = client.eval_repl('breakpoint set -n "FEngineLoop::Tick"', fid)
        ...
        msgs, hit = client.collect_until(
            ...
            timeout=10)
        if hit:
            p("  *** SOFTWARE BP on FEngineLoop::Tick HIT! ***")
            tid = (hit.get("body") or {}).get("threadId") or tid
            # Get new frame
            st_seq = client.request("stackTrace", {"threadId": tid, "startFrame": 0, "levels": 3})
            ...
            # Delete SW BP and test HW BP
            result, output = client.eval_repl("breakpoint delete", fid)
            p(f"  Deleted all BPs")

            # =====================================================
            # TEST 2: HW BP on FEngineLoop::Tick
            # =====================================================
            p("\n=== TEST 2: HW BP (-H) on FEngineLoop::Tick ===")
            result, output = client.eval_repl('breakpoint set -H -n "FEngineLoop::Tick"', fid)
            p(f"  {(output+result).strip()[:300]}")
            ...
            msgs, hit2 = client.collect_until(
                ...
                timeout=15)
            if hit2:
                p("  *** HW BP on FEngineLoop::Tick HIT! ***")
                p("  HW BPs persist after continue. Issue must be in VulkanCommandBuffer.cpp:334 specifically.")
            else:
                p("  *** HW BP on FEngineLoop::Tick DID NOT FIRE ***")
                p("  HW BPs do NOT persist after continue (debug registers cleared).")
                p("  Solution: use SOFTWARE breakpoints, which work after ASLR fix.")

                # Pause and do one more test: check if BP is still set
                pause_seq = client.request("pause", {"threadId": int(pid)})
                msgs, stop3 = client.collect_until(
                    lambda m: m.get("type") == "event" and m.get("event") == "stopped", timeout=10)
                if stop3:
                    tid3 = (stop3.get("body") or {}).get("threadId") or int(pid)
                    st_seq3 = client.request("stackTrace", {"threadId": tid3, "startFrame": 0, "levels": 1})
                    msgs, st3 = client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == st_seq3, timeout=10)
                    fid3 = None
                    if st3:
                        frs3 = (st3.get("body") or {}).get("stackFrames") or []
                        if frs3: fid3 = frs3[0]["id"]
                    result, output = client.eval_repl("breakpoint list", fid3)
                    p(f"\n  BP state after failed hit:\n  {(output+result).strip()[:400]}")
        else:
            p("  *** SOFTWARE BP on FEngineLoop::Tick ALSO DID NOT FIRE ***")
            p("  The game may not be running properly after attach.")
            # Pause and check thread state
            pause_seq = client.request("pause", {"threadId": int(pid)})
            ...
            if stop2:
                ...
                msgs, st2 = client.collect_until(lambda m: m.get("type") == "response" and m.get("request_seq") == st_seq2, timeout=10)
                fid2 = None
                if st2:
                    frs2 = (st2.get("body") or {}).get("stackFrames") or []
                    if frs2:
                        fid2 = frs2[0]["id"]
                        p(f"  Paused at: {frs2[0].get('name','?')}")
                result, output = client.eval_repl("thread list", fid2)
                p(f"\n  Thread list:\n  {(output+result).strip()[:1000]}")
